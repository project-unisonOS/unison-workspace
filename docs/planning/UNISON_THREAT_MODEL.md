...
| T-02 | Household member accesses another member's context | Phase 1 owner-derived namespaces and route binding cover the protected service inventory; context-space inference remains later | Principal-bound queries, row/space authorization, independent keys and indexes | P1 accepted evidence: cross-person read/write/search/cache/replay/object/vault/audit deny |
...
| T-15 | Secrets leakage | Hardened profile clears broad/static secrets and person-partitions connector credentials; developer-only defaults remain explicitly outside the product profile | Secret broker, no plaintext env in product, redaction, rotation, scanning, scoped injection | P1 accepted evidence: key/log canaries and hardened Compose scan; rotation remains |
...
| T-24 | Confused deputy between services | Phase 1 workload principals use explicit audiences and short person-preserving delegation; product profile disables broad static tokens | Workload identity plus end-user delegation chain and audience restriction | P1 accepted evidence: audience/delegation negative tests; purpose grants remain P3 |
...
| T-29 | Denial leaks protected existence | Principal mismatches are rejected before lookup with one generic response; deeper timing analysis remains | Non-oracular denial responses and privacy-preserving audit details | P1 accepted evidence: existing/nonexistent forged identifiers share denial; P4 timing proof remains |
