...
| T-21 | Supply-chain compromise | Phase 9 locks one artifact owner, deterministic digest manifest, Ed25519-signed bundle/bootstrap verification, installation receipts, threshold-signed update metadata, independently verified staging authorization, and checkpointed health-gated rollback; SBOM/provenance and public-download verification remain open | Pinned actions/digests, dependency locks, SBOM, signed provenance, image verification, reproducible builds | P9 bundle and update transactions reject artifact/metadata/root substitution and restore last known good after failure; P9.5 must verify public artifacts |
