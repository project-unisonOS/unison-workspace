...
Status: authoritative living threat model; initial audit baseline  
...
| --- | --- | --- | --- | --- |
| T-01 | Malicious or overprivileged household administrator | Phase 1 separates household administration from person data/key handles; host-root physical access remains a platform-hardening concern | Separate admin/data authority, per-person keys, minimal metadata, eviction without decryption, auditable admin operations | P1 candidate: admin has no private-read/key-export role; P4 proof remains |
| T-02 | Household member accesses another member's context | Phase 1 owner-derived namespaces and route binding cover the protected service inventory; context-space inference remains later | Principal-bound queries, row/space authorization, independent keys and indexes | P1 candidate: cross-person read/write/search/cache/replay/object/vault/audit deny |
| T-03 | Caller-supplied identity spoofing | Signed context and middleware now reject mismatched person/user/assistant/household/channel hints | Signed principal context, server-side assistant binding, reject mismatches | P1 candidate: forged-hint matrix and endpoint inventory pass |
...
| T-07 | Stolen appliance | Phase 1 provides per-person key handles, lock and revocation; disk encryption, TPM binding, remote revocation transport, and backup remain | Full-disk protection guidance, hardware-backed/per-person keys, lockout, remote token revocation, encrypted backups | P1 candidate: lock/revocation and encrypted local migration; P6 offline/backup proof remains |
| T-08 | Compromised connector/integration | Communications credentials and local stores are person-partitioned; fine-grained purpose/data grants remain Phase 3 | Per-person credential broker, scoped tokens, purpose/data grants, egress allowlists | P1 candidate: credential namespaces and audience tests; P3/P7 remain |
| T-09 | Prompt injection through email | Gmail messages can reach summarization/planning | Treat content as untrusted data, instruction/data separation, capability confirmation, provenance | P3/P7: malicious email cannot change policy or invoke capability |
| T-10 | Prompt injection through websites/documents/tools | Browser/VDI and research paths are early | Sandboxing, content labels, taint/provenance, no secret exposure, constrained tool plans | P3/P7: adversarial document cannot exfiltrate secrets |
| T-11 | Malicious capability package | Manifest supports trust and digest but permissions are incomplete | Signed package, provenance, explicit data/recipient declarations, sandbox, egress/file/device controls, revocation | P3/P8: capability exceeds manifest and is blocked/audited |
| T-12 | Overbroad model disclosure | Inference gateway can send arbitrary prompts to remote providers | Disclosure decision, local alternative check, minimization/redaction, provider terms profile, audit | P3: protected fields removed or call denied; external-call minimization metric |
...
| T-15 | Secrets leakage | Hardened profile clears broad/static secrets and person-partitions connector credentials; developer-only defaults remain explicitly outside the product profile | Secret broker, no plaintext env in product, redaction, rotation, scanning, scoped injection | P1 candidate: key/log canaries and hardened Compose scan; rotation remains |
| T-16 | Insecure logs and traces | Rich replay/telemetry can contain person IDs and payloads | Data-class-aware structured logging, payload references, per-person audit access, retention/deletion | P2/P3: sensitive fields redacted and private audit views isolated |
| T-17 | Metadata leakage | Provider can observe timing, sizes, identities; admins may see paths | Minimize identifiers, pad/batch where justified, document residual metadata, separate operational metrics | P5/P6: manifest lists exposed metadata and user disclosure |
| T-18 | Accidental shared-space promotion | No context-space model yet | Explicit copy/share action, preview, provenance, policy check, undo/tombstone | P2/P4: ingestion never changes space implicitly |
| T-19 | Cross-person inference leakage | Shared model/cache/process could leak embeddings or retrieved context | Per-principal retrieval/index/cache keys, prompt construction tests, memory isolation, no shared hidden state | P2/P4: canary secrets never appear in other assistant outputs |
...
| T-22 | Fail-open authorization dependency | Protected principal binding and revocation introspection now deny on auth/Redis failure; detailed degraded-read policy remains later | Fail-closed sensitive operations, safe degraded read-only modes, dependency health gates | P1 candidate: outage denial tests; P3 policy refinement remains |
| T-23 | Replay/idempotency attack | Some action idempotency exists; channels do not | Nonces, timestamps, idempotency store, provider event IDs, replay window | P3/P5: duplicate inbound event produces one authorized outcome |
| T-24 | Confused deputy between services | Phase 1 workload principals use explicit audiences and short person-preserving delegation; product profile disables broad static tokens | Workload identity plus end-user delegation chain and audience restriction | P1 candidate: audience/delegation negative tests; purpose grants remain P3 |
| T-25 | Shared infrastructure side channel | Shared databases, models, caches and traces | Namespace isolation, cache partitioning, resource quotas, constant/error-safe responses where needed | P4: timing/error/content tests and resource-abuse isolation |
| T-26 | Destructive or irreversible action | VDI/actuation/payment scaffolds exist | Risk/reversibility metadata, dry run, confirmation, undo/compensation, bounded authority | P3/P7: high-risk actions cannot execute without valid decision |
| T-27 | Coercive or manipulative optimization | No formal economic/product constraints in code | Personal charter, ranking constraints, no sponsored/engagement signals, inspectable rationale | P2/P7: objective provenance and product metric review |
| T-28 | Accessibility failure causes unsafe consent | Confirmations may differ by modality | Semantic parity, unambiguous recipient/data/action, cancellation and recovery in every modality | Every phase: equivalent accessible completion tests |
| T-29 | Denial leaks protected existence | Principal mismatches are rejected before lookup with one generic response; deeper timing analysis remains | Non-oracular denial responses and privacy-preserving audit details | P1 candidate: existing/nonexistent forged identifiers share denial; P4 timing proof remains |
| T-30 | Deletion/export incompleteness | Data spread across services, caches, traces, backups | Data inventory, tombstones, backup retention semantics, per-person export manifest | P2/P6: deletion/export reconciliation proves coverage |
