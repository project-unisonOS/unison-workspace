...
| --- | --- | --- | --- | --- |
| T-01 | Malicious or overprivileged household administrator | No household model; local host operator can inspect service files and global keys | Separate admin/data authority, per-person keys, minimal metadata, eviction without decryption, auditable admin operations | P1/P4: administrator denied private reads and key export |
| T-02 | Household member accesses another member's context | APIs accept person IDs; storage rows lack principal enforcement | Principal-bound queries, row/space authorization, independent keys and indexes | P1/P4: cross-person read/write/search/inference tests deny |
| T-03 | Caller-supplied identity spoofing | Optional `person_id`/`user_id` in envelopes and routes | Signed principal context, server-side assistant binding, reject mismatches | P1: mutate every identity hint and prove no authority change |
...
| T-07 | Stolen appliance | Global files/keys and plaintext fallback may expose data | Full-disk protection guidance, hardware-backed/per-person keys, lockout, remote token revocation, encrypted backups | P1/P6: offline data and backup remain unintelligible without keys |
| T-08 | Compromised connector/integration | Service-global credentials and broad context possible | Per-person credential broker, scoped tokens, purpose/data grants, egress allowlists | P1/P3/P7: connector cannot request other person's token/data |
...
| T-15 | Secrets leakage | Devstack defaults, env files, global Gmail store, logs | Secret broker, no plaintext env in product, redaction, rotation, scanning, scoped injection | P1/P3: secret canary absent from logs/model/capability output |
...
| T-22 | Fail-open authorization dependency | Auth blacklist explicitly allows when Redis is down; consent optional in services | Fail-closed sensitive operations, safe degraded read-only modes, dependency health gates | P1/P3: dependency outage denies protected operations |
...
| T-24 | Confused deputy between services | Static service tokens and broad service roles | Workload identity plus end-user delegation chain and audience restriction | P1/P3: service cannot reuse token for another person/purpose |
...
| T-29 | Denial leaks protected existence | Naive error can reveal another person's record | Non-oracular denial responses and privacy-preserving audit details | P1/P3/P4: existing/nonexistent private IDs are indistinguishable to attacker |
