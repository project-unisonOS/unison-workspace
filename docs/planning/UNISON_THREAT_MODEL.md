...
| T-17 | Metadata leakage | Telegram receives documented user/chat/bot/network/delivery metadata; gateway audit stores hashes and coarse outcomes without content | Minimize identifiers, pad/batch where justified, document residual metadata, separate operational metrics | P5 candidate: provider disclosure, content-free audit, isolated encrypted credentials/drafts; timing/size metadata remains residual risk |
...
| T-28 | Accessibility failure causes unsafe consent | Semantic decision, household, and remote-channel controls expose disclosure, pairing, step-up, cancellation, denial, revocation, and recovery without color-only meaning | Semantic parity, unambiguous recipient/data/action, cancellation and recovery in every modality | P5 candidate: labelled keyboard-native channel controls, live status, secret cleanup, denial, and recovery assertions pass; browser axe publication evidence pending |
